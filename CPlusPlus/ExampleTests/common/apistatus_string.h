...
struct ApiStatusString {
    std::string code;

    static ApiStatusString from_vec(const std::vector<std::string>& v) {
        ApiStatusString obj;
    ...
    static ApiStatusString from_text(const std::string& text) {
        const std::vector<std::string> parts = tokens::require(text, 1, "ApiStatus");
        ApiStatusString obj;
        obj.code = parts[0];
        return obj;
    }
    ...
    bool operator==(const ApiStatusString& o) const {
        return dnc_equal(code, o.code);
    }
    bool operator!=(const ApiStatusString& o) const { return !(*this == o); }
};
