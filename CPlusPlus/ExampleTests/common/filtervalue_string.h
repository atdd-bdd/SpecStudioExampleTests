...
struct FilterValueString {
    ...
    static FilterValueString from_text(const std::string& text) {
        const std::vector<std::string> parts = tokens::require(text, 1, "FilterValue");
        FilterValueString obj;
        obj.value = parts[0];
        return obj;
    }
    ...
    bool operator==(const FilterValueString& o) const {
        return dnc_equal(value, o.value);
    }
    bool operator!=(const FilterValueString& o) const { return !(*this == o); }
};
