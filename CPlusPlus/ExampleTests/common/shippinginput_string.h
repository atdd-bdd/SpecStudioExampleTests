#pragma once
#include <cctype>
#include <string>
#include <vector>
#include <sstream>
#include "tokens.h"

#ifndef SPECTABLE_DNC_STRING
#define SPECTABLE_DNC_STRING
inline const std::string DNCString = "?DNC?";
inline bool dnc_equal(const std::string& a, const std::string& b) {
    return a == b || a == DNCString || b == DNCString;
}
// Reads the Yes/No/True/False text a spec cell may hold, in any casing.
inline bool parse_bool_cell(const std::string& v) {
    std::string t;
    for (char c : v) t += static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
    return t == "true" || t == "t" || t == "yes" || t == "y" || t == "1";
}
#endif

struct ShippingInputString {
    std::string total_price;
    std::string shipping_cost;
    std::string notes;

    static ShippingInputString from_vec(const std::vector<std::string>& v) {
        ShippingInputString obj;
        if (v.size() > 0) obj.total_price = v[0];
        if (v.size() > 1) obj.shipping_cost = v[1];
        if (v.size() > 2) obj.notes = v[2];
        return obj;
    }

    /// Builds from the text form, e.g. Money as "25 USD".
    static ShippingInputString from_text(const std::string& text) {
        const std::vector<std::string> parts = tokens::require(text, 3, "ShippingInput");
        ShippingInputString obj;
        obj.total_price = parts[0];
        obj.shipping_cost = parts[1];
        obj.notes = parts[2];
        return obj;
    }

    std::string to_string() const {
        std::ostringstream ss;
        ss << tokens::token(total_price);
        ss << " ";
        ss << tokens::token(shipping_cost);
        ss << " ";
        ss << tokens::token(notes);
        return ss.str();
    }

    bool operator==(const ShippingInputString& o) const {
        return dnc_equal(total_price, o.total_price)
            && dnc_equal(shipping_cost, o.shipping_cost)
            && dnc_equal(notes, o.notes);
    }
    bool operator!=(const ShippingInputString& o) const { return !(*this == o); }
};
