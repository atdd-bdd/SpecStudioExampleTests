...
struct ItemPriceInputString {
    std::string totalitems;

    static ItemPriceInputString from_vec(const std::vector<std::string>& v) {
        ItemPriceInputString obj;
        if (v.size() > 0) obj.totalitems = v[0];
        return obj;
    }

    /// Builds from the text form, e.g. Money as "25 USD".
    static ItemPriceInputString from_text(const std::string& text) {
        const std::vector<std::string> parts = tokens::require(text, 1, "ItemPriceInput");
        ItemPriceInputString obj;
    ...
    bool operator==(const ItemPriceInputString& o) const {
        return dnc_equal(totalitems, o.totalitems);
    }
    bool operator!=(const ItemPriceInputString& o) const { return !(*this == o); }
};
