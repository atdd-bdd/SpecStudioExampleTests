...
struct PostString {
    ...
    static PostString from_vec(const std::vector<std::string>& v) {
        PostString obj;
        if (v.size() > 0) obj.userid = v[0];
        if (v.size() > 1) obj.id = v[1];
    ...
    static PostString from_text(const std::string& text) {
        const std::vector<std::string> parts = tokens::require(text, 4, "Post");
        PostString obj;
        obj.userid = parts[0];
        obj.id = parts[1];
        obj.title = parts[2];
    ...
    bool operator==(const PostString& o) const {
        return dnc_equal(userid, o.userid)
            && dnc_equal(id, o.id)
            && dnc_equal(title, o.title)
            && dnc_equal(body, o.body);
    }
    bool operator!=(const PostString& o) const { return !(*this == o); }
};
