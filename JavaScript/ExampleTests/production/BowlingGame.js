 ...
 */
export class Frame {
  constructor(number, roll1, roll2, roll3, score, totalScore) {
    this.number = number;
    this.roll1 = roll1;
    this.roll2 = roll2;
    this.roll3 = roll3;
...
export class BowlingGame {
   ...
   */
  #frameStarts() {
    const starts = new Array(FRAMES + 1).fill(0);

    if (this.#tenthFrameOnly) {
      // Frames 1..9 are unbowled: point them past every roll so each one reads
      // back as TBR.
      for (let f = 1; f < FRAMES; ++f) starts[f] = this.#rolls.length + FRAMES * 2;
      starts[FRAMES] = 0;
      return starts;
    }
    ...
    for (let f = 1; f < FRAMES; ++f) {
      starts[f] = index;
      index += (index < this.#rolls.length && this.#rolls[index] === Pins.MAX) ? 1 : 2;
    }
    starts[FRAMES] = index;
    return starts;
  }
  ...
  #pinsAt(index) {
    if (index < 0 || index >= this.#rolls.length) return Pins.TBR;
    return new Pins(this.#rolls[index]);
  }

  #allRolled(start, count) {
