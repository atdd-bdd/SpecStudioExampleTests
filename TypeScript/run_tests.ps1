# Run the TypeScript tests from the command line.
#
...
function Need($name, $fallback, $hint) {
    ...
    Write-Host "TypeScript: $name not found. $hint" -ForegroundColor Yellow
    exit 127
}

$npm = Need 'npm' $null 'Install Node.js.'
if (-not (Test-Path 'node_modules')) {
    Write-Host 'Installing dependencies (first run only)...'
    & $npm install --silent
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
# npm test compiles with tsc first, then runs Jest over the emitted JavaScript.
& $npm test
exit $LASTEXITCODE
