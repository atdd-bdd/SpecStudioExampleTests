...
impl DiscountingTyped {
    ...
    pub fn to_json_list(list: &[DiscountingTyped]) -> String {
        json::write(&json::Value::Array(
            list.iter().map(|item| item.to_json_value()).collect(),
        ))
    }

    pub fn from_json_list(text: &str) -> json::JsonResult<Vec<DiscountingTyped>> {
        let root = json::parse(text)?;
        let items = json::as_array(&root, "DiscountingTyped")?;
