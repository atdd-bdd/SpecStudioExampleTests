# Run the Rust tests from the command line.
#
...
function Need($name, $fallback, $hint) {
    ...
    Write-Host "Rust: $name not found. $hint" -ForegroundColor Yellow
    exit 127
}

$cargo = Need 'cargo' "$env:USERPROFILE\.cargo\bin\cargo.exe" 'Install Rust via rustup.'
& $cargo test
exit $LASTEXITCODE
