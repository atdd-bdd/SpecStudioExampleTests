# Run the Go tests from the command line.
#
...
function Need($name, $fallback, $hint) {
    ...
    Write-Host "Go: $name not found. $hint" -ForegroundColor Yellow
    exit 127
}

$go = Need 'go' 'C:\Program Files\Go\bin\go.exe' 'Install Go.'
# -count=1 disables the result cache, so a demo always really runs the tests.
& $go test -count=1 -v ./...
exit $LASTEXITCODE
